...
### Replaced buffered.increment.enabled property with view.counts.enabled property
- **Date:** 2020-Oct-21
- **JIRA Ticket:** [LPS-122159](https://issues.liferay.com/browse/LPS-122159)

#### What changed?

The portal property `buffered.increment.enabled` has been replaced by the `view.counts.enabled` property in `portal.properties`.

#### Who is affected?

This affects anyone who has set the `buffered.increment.enabled` property, or
overrode it for the `AssetEntry`, `DLFileEntry`, `DLFolderEntry` and `MBThread` entities to `false`.

#### How should I update my code?

The `view.counts.enabled`, `view.count.enabled[AssetEntry]`, `view.count.enabled[DLFileEntry]` and `view.count.enabled[MBThread]` properties can be found in `portal.properties`.

#### Why was this change made?

Setting `buffered.increment.enabled[DLFolderEntry]` to `false`, during
multiple file upload, `DLFolderLocalServiceImpl.updateLastPostDate()` was called in succession for the same temporary folder leading to a `org.hibernate.StaleObjectStateException`.
